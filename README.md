...
- **QKV** - 이게 가장 헷갈렸던 부분이다. 각 Query, Key, Value가 무엇을 의미하는지, 왜냐하면 모두 똑같은 값을 나누었기 때문에… transpose의 역할을 제대로 이해 못했던 순간도 있었다. 간단한데도…
- **Multi Head Attention** - 재밌는 방식이었다. Head가 하나의 공간을 얼마나 나누어 볼 것인지 정하는 것인지,
- **FFNN** - 얘는 어렵지 않았었다. 그냥 차원을 늘렸다가 줄이는 방식으로, 디테일만 좀 더 살펴보는 느낌으로 이해하면 됐다.

- **기타 등등…**
    - Embedding(token-position), logits, Linearization(선형 변환), 잔차계산(Residual), dropout,   LayerNormalization(scale-shift), 활성 함수(Activation Function), softmax, transpose, Logits 선택 알고리즘 etc…
...
Notion: https://www.notion.so/27e8875785bf81ef9093c8199248eff5
Figma: https://www.figma.com/board/3fBAFmTEjzej86ENCTZKye/LLM?node-id=0-1&t=Kpi4lQs37y56Uikt-1
